...
│                          │                              │
├──────────────────────────┴──────────────────────────────┤
│  状态栏：行 12，列 34  |  字符 1,024  |  286 词  |  已保存  │
└─────────────────────────────────────────────────────────┘
```
...
- 查找/替换：Cmd+F 打开浮动工具栏（仿 Xcode 风格，覆盖在编辑器上方，Escape 关闭）
...
**最近文件**：通过 `NSDocumentController.shared.noteNewRecentDocumentURL()` 接入系统"打开最近文件"子菜单，`.onOpenURL` 处理从菜单点击打开的文件。
...
| 撤销/重做 | Cmd+Z / Cmd+Shift+Z | NSTextView 自带 |
| 查找/替换 | Cmd+F / Cmd+H | 浮动工具栏（Xcode 风格） |
| 切换预览 | Cmd+Shift+P | 隐藏/显示右侧面板；右下角按钮同效 |
...
### Phase 1 — 核心链路（1~2 周）✅ 已完成
- [x] 创建项目，配置 entitlements（`com.apple.security.files.user-selected.read-write`）
- [x] `EditorView`：NSTextView 桥接，文本双向绑定
- [x] `PreviewView`：WKWebView 加载本地模板，注入 marked.js
- [x] 防抖触发渲染，验证实时联动
- [x] `DocumentViewModel`：content / fileURL / isModified

### Phase 2 — 文件管理（3~5 天）✅ 已完成
- [x] 新建 / 打开 / 保存 / 另存为
- [x] 窗口标题显示文件名，未保存显示 `●`
- [x] 关闭前 unsaved 确认弹窗

### Phase 3 — 收尾（3~5 天）✅ 已完成
- [x] Light/Dark 预览 CSS 跟随系统外观
- [x] 状态栏：字符数、光标位置
- [x] 大文件 / 特殊字符 / 中文输入测试

### Phase 4 — 功能扩展（进行中）
- [ ] 切换预览面板：Cmd+Shift+P + 右下角浮动按钮（`sidebar.right` 图标）
- [ ] 状态栏字数统计：新增"词数"和"行数"字段
- [ ] 最近文件菜单：接入系统 `NSDocumentController` Open Recent 子菜单
- [ ] 查找/替换浮动工具栏：Cmd+F 打开，支持查找下一个/上一个、替换、全部替换
- [ ] 补全单元测试：预览切换、字数统计、查找/替换逻辑
