...
**文件**：`Views/FindBarView.swift`

**职责**：浮动查找/替换工具栏，通过 `ZStack` 覆盖在 `EditorView` 顶部，条件显示（`showFindBar == true`）。

**布局**：
```
┌─────────────────────────────────────────────┐
│ 🔍 [查找...          ]  ← →  ✕             │  ← 毛玻璃背景 .ultraThinMaterial
│ ↩  [替换...          ]  替换  全部替换      │
└─────────────────────────────────────────────┘
```
...
struct FindBarView: View {
    @ObservedObject var vm: DocumentViewModel
    var onSelectRange: ((NSRange) -> Void)    // 通知 EditorView 高亮选中
}
```

**实现要点**：
- `vm.findNext()` / `vm.findPrevious()` 在 `vm.content` 中定位匹配，返回 `NSRange`
- 通过 `onSelectRange` 回调通知 `EditorView.Coordinator` 调用 `NSTextView.setSelectedRange(_:)`
- `vm.replaceAll()` 直接修改 `vm.content`，触发 Combine 链更新预览
- Cmd+F 打开，Escape 关闭，Return / Shift+Return 查找下一个/上一个

**与 ContentView 集成**：
```swift
ZStack(alignment: .top) {
    EditorView(..., onSelectRange: { range in ... })
    if vm.showFindBar {
        FindBarView(vm: vm, onSelectRange: { ... })
...
| 模块 | 对外暴露 | 依赖 |
|------|---------|------|
| EditorView | `@Binding var text`，`onCursorMove` 回调，`onSelectRange` 回调 | NSTextView, AppKit |
| PreviewView | `var markdown: String`，`var isDark: Bool` | WKWebView, WebKit |
| StatusBarView | `@ObservedObject var vm` | DocumentViewModel |
| FindBarView | `@ObservedObject var vm`，`onSelectRange` 回调 | DocumentViewModel |
| DocumentViewModel | `@Published` 属性，`open/save/saveAs/newDocument/findNext/replaceAll` | FileService, Combine |
...
└── MarkdownEditor/
    ...
    │   └── FindBarView.swift              # 浮动查找/替换工具栏（ZStack 覆盖编辑器顶部）
    │
    ├── ViewModels/
